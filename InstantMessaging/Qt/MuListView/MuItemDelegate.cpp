...
void MuItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    if (index.isValid()) {
        ...
        // 绘制图片，歌手，数量位置区域
        QRectF iconRect = QRectF(rect.left()+5, rect.top()+5, 40, 40);
        QRectF singerRect = QRectF(iconRect.right()+5, iconRect.top(), rect.width()-10-iconRect.width(), 20);
        QRectF songNbRect = QRectF(singerRect.left(), singerRect.bottom()+5., rect.width()-10-iconRect.width(), 20);
